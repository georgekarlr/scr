...
const RushBreakGame: React.FC<RushBreakGameProps> = ({ setId, selectedItemIds, duration: initialDuration }) => {
  ...
  } = useRushBreakGame({ setId, selectedItemIds, initialDuration });
      ...
      <div className="w-full bg-white border-2 border-purple-100 rounded-3xl p-5 sm:p-8 shadow-xl">
        ...
        {!isAnswered && (
          <div className="flex space-x-2">
            ...
            <button onClick={handleCheck} disabled={Object.keys(matchingMatches).length < content.pairs.length} className="flex-[2] py-3 bg-purple-600 text-white font-bold rounded-2xl shadow-lg shadow-purple-100 hover:bg-purple-700 transition-all disabled:opacity-50 text-sm">Check Matches</button>
          </div>
        )}
        {isAnswered && content.explanation && (
          <div className="mt-6 p-4 bg-gray-50 rounded-2xl border border-gray-100 animate-in slide-in-from-top-2 duration-300">
      ...
      <div className="w-full bg-white border-2 border-blue-100 rounded-3xl p-5 sm:p-8 shadow-xl">
        ...
        <div className="space-y-2 sm:space-y-3 mb-6">
          {orderedItems.map((text, idx) => {
            ...
            return (
              <div key={idx} className={`flex items-center justify-between p-3 sm:p-4 rounded-2xl border-2 transition-all ${isAnswered ? (isCorrectPosition ? 'border-green-500 bg-green-50 text-green-700' : 'border-red-500 bg-red-50 text-red-700') : 'border-blue-50 bg-blue-50 text-blue-900'}`}>
                <div className="flex items-center space-x-3 sm:space-x-4 min-w-0">
                  <div className={`h-8 w-8 rounded-full flex-shrink-0 flex items-center justify-center font-bold text-sm ${isAnswered ? (isCorrectPosition ? 'bg-green-600' : 'bg-red-600') : 'bg-blue-600'} text-white`}>{idx + 1}</div>
                  <span className="font-medium text-base truncate">{text}</span>
                </div>
                {!isAnswered && (
                  <div className="flex flex-col space-y-1">
                    <button onClick={() => moveItem(idx, 'up')} disabled={idx === 0} className="p-1 hover:bg-blue-100 rounded-lg disabled:opacity-30"><ChevronLeft className="h-4 w-4 rotate-90" /></button>
                    <button onClick={() => moveItem(idx, 'down')} disabled={idx === orderedItems.length - 1} className="p-1 hover:bg-blue-100 rounded-lg disabled:opacity-30"><ChevronRight className="h-4 w-4 rotate-90" /></button>
                  </div>
                )}
              </div>
            );
        ...
        {!isAnswered && <button onClick={handleCheck} className="w-full py-3 sm:py-4 bg-blue-600 text-white font-bold rounded-2xl shadow-lg shadow-blue-100 hover:bg-blue-700 transition-all active:scale-95 text-sm sm:text-base">Check Order</button>}
        {isAnswered && content.explanation && (
          <div className="mt-6 p-4 bg-gray-50 rounded-2xl border border-gray-100 animate-in slide-in-from-top-2 duration-300">
            <p className="text-sm font-bold text-gray-900 mb-1">Explanation</p>
            <p className="text-sm text-gray-600 italic">{content.explanation}</p>
  ...
  const renderCheckboxQuestion = (content: CheckboxQuestionContent) => {
    ...
    return (
      <div className="w-full bg-white border-2 border-indigo-100 rounded-3xl p-5 sm:p-8 shadow-xl">
        ...
        <div className="space-y-2 sm:space-y-3 mb-6">
          {shuffledCheckboxOptions.map((opt) => {
            ...
            if (isAnswered) {
              ...
              buttonClass += isSelected ? "border-indigo-500 bg-indigo-50 text-indigo-700" : "border-gray-50 bg-gray-50 hover:border-indigo-300 hover:bg-indigo-50 text-gray-700";
            }
            return (
              <button 
                key={opt.id}
              ...
              >
                <span className="min-w-0 flex-1 mr-2">{opt.text}</span>
                <div className={`h-5 w-5 sm:h-6 sm:w-6 rounded-lg border-2 transition-colors flex-shrink-0 flex items-center justify-center ${isAnswered && isCorrect ? 'border-green-500 bg-green-500' : isAnswered && isSelected ? 'border-red-500 bg-red-500' : isSelected ? 'border-indigo-500 bg-indigo-500' : 'border-gray-200 group-hover:border-indigo-500'}`}>
        ...
        {!isAnswered && <button onClick={handleCheck} disabled={selectedOptionIds.length === 0} className="w-full py-3 sm:py-4 bg-indigo-600 text-white font-bold rounded-2xl shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all disabled:opacity-50 text-sm sm:text-base">Check Answers</button>}
        {isAnswered && content.explanation && (
          <div className="mt-6 p-4 bg-gray-50 rounded-2xl border border-gray-100 animate-in slide-in-from-top-2 duration-300">
            <p className="text-sm font-bold text-gray-900 mb-1">Explanation</p>
            <p className="text-sm text-gray-600 italic">{content.explanation}</p>
    ...
    return (
      <div className="w-full bg-white border-2 border-pink-100 rounded-3xl p-5 sm:p-8 shadow-xl">
        ...
        <div className="space-y-4">
          <input type="text" value={writtenAnswer} onChange={(e) => setWrittenAnswer(e.target.value)} disabled={isAnswered} placeholder="Type your answer here..." className={`w-full p-3 sm:p-4 rounded-2xl border-2 transition-all font-medium focus:outline-none text-sm sm:text-base ${isAnswered ? (isCorrect ? 'border-green-500 bg-green-50 text-green-700' : 'border-red-500 bg-red-50 text-red-700') : 'border-gray-50 bg-gray-50 text-gray-900 focus:border-pink-500 focus:bg-pink-50'}`} onKeyDown={(e) => e.key === 'Enter' && writtenAnswer.trim() && !isAnswered && handleCheck()} />
          {!isAnswered && <button onClick={handleCheck} disabled={!writtenAnswer.trim()} className="w-full py-3 sm:py-4 bg-pink-600 text-white font-bold rounded-2xl shadow-lg shadow-pink-100 hover:bg-pink-700 transition-all disabled:opacity-50 text-sm sm:text-base">Submit Answer</button>}
          {isAnswered && (
            <div className={`p-4 rounded-2xl border-2 ${isCorrect ? 'border-green-200 bg-green-50' : 'border-red-200 bg-red-50'}`}>
              ...
              <div className="flex flex-wrap gap-2">
                {content.accepted_answers.map((ans, idx) => <span key={idx} className="px-3 py-1 bg-white rounded-lg text-xs font-bold text-pink-600 border border-pink-200">{ans}</span>)}
              </div>
            </div>
          )}
        </div>
      ...
      <div className="w-full max-w-2xl px-4 flex flex-col items-center">
        <div className="w-full flex items-center justify-between mb-8 bg-gray-800/50 backdrop-blur-md p-4 rounded-2xl border border-white/10">
          <div className="flex items-center space-x-3">
            <Clock className={`h-6 w-6 ${timeLeft <= 10 ? 'text-red-400 animate-pulse' : 'text-blue-400'}`} />
            <span className={`text-2xl font-black tabular-nums ${timeLeft <= 10 ? 'text-red-400' : 'text-white'}`}>{timeLeft}s</span>
          </div>
        ...
        <div className="w-full animate-in slide-in-from-bottom-4 duration-500">
          {currentItem.type === 'flashcard' && (
            <div className="space-y-6">
              ...
              {flipped && <button onClick={() => handleAnswer(true)} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-xl transition-all active:scale-[0.98] animate-in fade-in slide-in-from-top-2 duration-300">Continue</button>}
            </div>
          )}
          ...
          {currentItem.type === 'note' && (
            <div className="space-y-6">
              ...
              <button onClick={() => handleAnswer(true)} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-xl transition-all active:scale-[0.98]">Got it!</button>
            </div>
          )}
    ...
    <div className="w-full max-w-xl bg-white rounded-3xl p-10 shadow-2xl text-gray-900 animate-in zoom-in duration-300">
      <div className="text-center">
        ...
        {gameEndedReason !== 'no-items' && (
          <>
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div className="bg-blue-50 rounded-2xl p-6 relative overflow-hidden group">
                <div className="absolute -right-2 -bottom-2 text-blue-100 group-hover:scale-110 transition-transform duration-500">
                  <Zap size={64} fill="currentColor" />
                </div>
                <span className="text-blue-400 font-bold uppercase tracking-widest text-xs mb-1 block relative z-10">XP Earned</span>
                {isFinishing ? (
                  <Loader2 className="h-8 w-8 text-blue-600 animate-spin relative z-10" />
                ) : (
                  <span className="text-4xl font-black text-blue-700 relative z-10">
        ...
        <div className="space-y-3">
          <button onClick={() => setGameState('config')} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-xl shadow-blue-100 transition-all flex items-center justify-center space-x-2"><RefreshCw className="h-5 w-5" /><span>Play Again</span></button>
