...
import { Users, Search, Filter, Plus, GraduationCap, Mail, School, X, BookOpen } from 'lucide-react';
...
const StudentRoster: React.FC = () => {
  ...
  const handleOpenModal = (user?: UserProfile) => {
    if (user) {
      setEditingUser(user);
      setFormData({
        email: '', // Not available in UserProfile from RPC
        password: '',
  ...
  const filteredStudents = students.filter(u => 
    `${u.user_metadata?.first_name} ${u.user_metadata?.last_name}`.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Master Roster</h1>
          <p className="text-gray-600">View and manage all students enrolled in the school.</p>
        </div>
        <button 
...
const StudentRow: React.FC<{ user: UserProfile; onEdit: () => void }> = ({ user, onEdit }) => (
  <tr className="hover:bg-gray-50 transition-colors">
    ...
    <td className="px-6 py-4">
      <span className="flex items-center gap-1.5 text-sm text-gray-600 font-mono">
        {user.school_id || 'N/A'}
      </span>
    </td>
    <td className="px-6 py-4">
